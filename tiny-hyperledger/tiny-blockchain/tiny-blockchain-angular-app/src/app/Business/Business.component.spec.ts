...
describe('BusinessComponent', () => {
  ...
  it('should update the table when a Business is added', fakeAsync(() => {
    ...
    sinon.stub(component.serviceBusiness, 'addParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Business is updated', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceBusiness, 'updateParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
  ...
  it('should update the table when a Business is deleted', fakeAsync(() => {
    let loadAllSpy = sinon.stub(component, 'loadAll');
    sinon.stub(component.serviceBusiness, 'deleteParticipant').returns(new Observable(observer => {
      observer.next('');
      observer.complete();
    }));
